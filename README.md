...
使用 Gemini 提炼带证据的量化案例，并写入 SQLite 新闻/案例数据库。
...
- Gemini 输出摘要、量化案例、证据原文、企业、地区、指标标签和相关性。
...
打开 `.env`，通过 `GEMINI_API_KEY` 填入你自己的 Gemini API Key。
不要将 `.env` 提交或发送给其他人。
...
- 每轮默认最多处理 2 个主题，避免集中消耗 Gemini 配额。
...
测试使用临时 SQLite 数据库，不请求新闻站点，也不调用 Gemini。
